...
def attacker_uncertainty_update(att_in_system_time, att_detect, dec, uncertain_scheme, decision_scheme,
    ...
    uncertainty = 1 - math.exp(-(df * communication_factor) / att_in_system_time)
...
# Return: ai_{ik}
def attack_impact(G, new_compromised_list, _Omeka):
    _Omeka = 0
    ...
    for n in new_compromised_list:
        sum_em_ag += G.nodes[n].get("A", 0)  # Sum of  A
    ...
    ci = total_criticality / N

    ai = _Omeka * part1 + (1 - _Omeka) * ci
...
def attacker_class_choose_strategy(self, def_strategy_number,
    ...
    # eq. 17
    self.expected_impact = exp_att_impact_of_all_stra(self.network, self.location, self.strategy_number,
    ...
    elif self.decision_scheme == 1 or self.decision_scheme == 2 or self.decision_scheme == 6:  # HEU-based selection scheme
...
class attacker_class:
    ...
    def update_attribute(self, dec, bandwidth_usage, latency, packet_loss_rate):
        # monitor time
        self.monit_time += 1
        self.defender_monit_time += 1
        ...
        self.uncertainty = attacker_uncertainty_update(self.monit_time,
                                                       ...
                                                       self.uncertain_scheme, self.decision_scheme, bandwidth_usage,
                                                       latency, packet_loss_rate
                                                       )
        self.defender_uncertainty = defender_uncertainty_update(self.detect_prob, self.defender_monit_time,
                                                                self.defender_uncertain_scheme,
                                                                self.defender_decision_scheme, bandwidth_usage, latency,
                                                                packet_loss_rate
                                                                )
    ...
    def def_guess_att_EU_C(self):
        ...
        if self.NIDS_detected:
            xi = 5
            def_guess_att_impact = exp_att_impact_of_all_stra(self.game.defender.network, self.location,
                                                              self.strategy_number, self.compromised_nodes)
