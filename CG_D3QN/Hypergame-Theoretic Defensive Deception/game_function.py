...
# In[25]:
import graph_function as update_vul
from graph_function import *

from attacker_function import *
from defender_function_new import *
import main as display
...
class game_class:
    def __init__(self,
        ...
        # D3QN using data
        self.D3QN_state_data = []
        self.D3QN_action_data = []
        self.D3QN_reward_data = []
    ...
    def attacker_round(self, simulation_id, bandwidth_usage, latency,packet_loss_rate):
        ...
        for attacker in self.attacker_list:
            ...
            attacker.update_attribute(self.defender.dec,bandwidth_usage, latency,packet_loss_rate)
            self.update_graph()
            if attack_result:
    ...
    def experiment_saving(self):
        ...
        self.D3QN_state_data.append(data_x.tolist())  # 直接使用ML的数据作为状态数据

        # for index in range(len(data_name)):
        #     print(f"{index} : {data_name[index]}")

        # save data for train D3QN
        # save data x:
        # data_state = []
        # data_state_name = []
        #
        # # 添加不同的数据源到 data_state
        # data_state = np.concatenate((data_state, self.defender.att_previous_strat))
        # data_state_name += ["att_str"] * len(self.defender.att_previous_strat)
        #
        # data_state = np.concatenate((data_state, self.attacker_template.strat_cost))
        # data_state_name += ["att_cost"] * len(self.attacker_template.strat_cost)
        #
        # data_state = np.concatenate((data_state, self.defender.strat_cost))
        # data_state_name += ["def_cost"] * len(self.defender.strat_cost)
        #
        # data_state = np.concatenate((data_state, self.att_previous_impact))  # from index 24 to 31
        # data_state_name += ["att_impact"] * len(self.att_previous_impact)
        #
        # data_state = np.concatenate((data_state, [self.att_previous_overall_impact]))  # index 32
        # data_state_name += ["att_OvAll_impact"]
        #
        # data_state = np.concatenate((data_state, self.def_previous_impact))  # from index 33 to 40
        # data_state_name += ["def_impact"] * len(self.def_previous_impact)
        #
        # data_state = np.concatenate((data_state, [self.previous_uncertain]))
        # data_state_name += ["uncert"]
        #
        # data_state = np.concatenate((data_state, self.defender.att_previous_CKC))
        # data_state_name += ["CKC"] * len(self.defender.att_previous_CKC)
        #
        #
        #
        #
        ...
        #
        #
        # data_state = np.concatenate((data_state, self.defender.monit_time))
        # data_state_name += ["def_monit_time"] * len(self.defender.monit_time)
        #
        # data_state = np.concatenate((data_state, self.DHEU_per_Strategy_History))
        # data_state_name += ["DHEU_str_his"] * len(self.DHEU_per_Strategy_History)
        #
        # data_state = np.concatenate((data_state, self.AHEU_per_Strategy_History))
        # data_state_name += ["AHEU_str_his"] * len(self.AHEU_per_Strategy_History)
        #
        # data_state = np.concatenate((data_state, self.att_uncertainty_history))
        # data_state_name += ["att_uncer_his"] * len(self.att_uncertainty_history)
        #
        # data_state = np.concatenate((data_state, self.def_cost_history))
        # data_state_name += ["def_cost_his"] * len(self.def_cost_history)
        #
        # data_state = np.concatenate((data_state, self.att_cost_history))
        # data_state_name += ["att_cost_his"] * len(self.att_cost_history)
        #
        # data_state = np.concatenate((data_state, self.def_strategy_counter))
        # data_state_name += ["def_str_count"] * len(self.def_strategy_counter)
        #
        # data_state = np.concatenate((data_state, self.def_HEU_history))
        # data_state_name += ["def_HEU_his"] * len(self.def_HEU_history)
        #
        # data_state = np.concatenate((data_state, self.def_uncertainty_history))
        # data_state_name += ["def_HEU_his"] * len(self.def_uncertainty_history)
        #
        #
        # # 保存数据用于训练
        # self.D3QN_state_data.append(data_state.tolist())  # 直接使用 ML 的数据作为状态数据
        #
        # # 准备行动数据
        # data_action = []
        # data_action_name = []
        # data_action += self.defender.chosen_strategy_list
        # data_action_name += ["chosen_strategy_list"] * len(self.defender.strategy_number)
        #
        # # 准备奖励数据
        # data_reward = []
        # data_reward_name = []
        # data_reward += self.defender.DHEU
        # data_reward_name += ["DHEU"] * len(self.defender.DHEU)
        #
        # # 保存行动和奖励数据
        # self.D3QN_action_data.append(data_action)
        # self.D3QN_reward_data.append(data_reward)
            ...
            for j in range(self.strategy_number):
                utility[i, j] = (self.defender.impact_record[i] +
                                 self.attacker_template.strat_cost[j] / 3) - (
                                        self.defender.strat_cost[i] / 3 + attack_impact_record[j])#*math.exp((-x)* attack_impact_record[j]))
        ...
        self.D3QN_reward_data.append(max_value)
        self.D3QN_action_data.append(random.choice(max_indexs))
