...
from ML_train_D3QN_env import env
from ML_training_D3QN_Bing import D3QN
...
def defender_uncertainty_update(att_detect, def_monit_time, uncertain_scheme, decision_scheme,
                                bandwidth_usage, latency, packet_loss_rate):
    mu = 10  # was 1
    communication_factor = (1 + bandwidth_usage + latency + packet_loss_rate) / 3

    uncertainty = 1 - math.exp((-mu) * att_detect* communication_factor / def_monit_time)
...
def defender_class_choose_bundle(self, att_strategy_number, attack_cost_record, attack_impact_record, vary_name,
                                 vary_value, att_overall_impact, attacker_list,
                                 ):
    global y_pred_dic
        ...
        for j in range(att_strategy_number):
            ...
            j] = (self.impact_record[i] +
                  attack_cost_record[j] / 3) - (
                         self.strat_cost[i] / 3 + attack_impact_record[j])  # *math.exp((-x)* overall_ai))

    # eq. 8  计算了一个一维数组EU_C，用于表示防御者每个策略的预期效用值。
    ...
    # 使用D3QN算法，进行策略选择
    if self.decision_scheme == 6:
        # use trained model
        if vary_name is None:
            # fixed setting
            if self.uncertain_scheme:  # when consider uncertianty
                # print("using IPI")
                the_file = open("data/trained_D3QN_model/trained_D3QN_model_D3QN_collect_data_IPI.pkl", "rb")
            else:  # when not consider uncertainty
                the_file = open("data/trained_D3QN_model/trained_D3QN_model_D3QN_collect_data_PI.pkl", "rb")
        else:
            # varying settings
            if self.uncertain_scheme:  # when consider uncertianty
                the_file = open("data_vary/" + vary_name + "=" + str(
                    vary_value) + "/trained_D3QN_model/trained_D3QN_model_D3QN_collect_data_IPI.pkl", "rb")
            else:  # when not consider uncertainty
                the_file = open("data_vary/" + vary_name + "=" + str(
                    vary_value) + "/trained_D3QN_model/trained_D3QN_model_D3QN_collect_data_PI.pkl", "rb")

        # try: #先不注释
        # the_file = 'path_to_model_file'
        # except EOFError:
        #    exit(1)

        D3QN_model = torch.load(the_file, map_location='cpu')
        the_file.close()
        data_x = []
        data_x = np.concatenate((data_x, self.att_previous_strat))  # 环境
        data_x = np.concatenate((data_x, attack_cost_record))
        data_x = np.concatenate((data_x, self.strat_cost))
        data_x = np.concatenate((data_x, attack_impact_record))  # from index 24 to 31
        data_x = np.concatenate((data_x, [att_overall_impact]))  # index 32
        ...
        # 假设已经初始化了D3QN模型，其中包括primary_network和target_network
        #
        # 获取当前状态
        state = data_x
        D3QN_bundle = D3QN_model.predict_strategy_bundle(state)
        action = D3QN_model.act(state)
        D3QN_first_opt = D3QN_bundle[0]
        D3QN_second_opt = D3QN_bundle[1]

        if self.strat_cost[D3QN_first_opt] + self.strat_cost[
            D3QN_second_opt] <= self.cost_limit and D3QN_first_opt != D3QN_second_opt and D3QN_second_opt != 3:
            self.chosen_strategy_list = D3QN_bundle
        else:
            self.chosen_strategy_list = action
        ...
        if vary_name is None:
            ...
            if self.uncertain_scheme:  # when consider uncertianty
                the_file = open("data_vary/" + vary_name + "=" + str(
                    vary_value) + "/trained_ML_model/trained_classi_model_ML_collect_data_IPI.pkl", "rb")
            else:  # when not consider uncertainty
                the_file = open("data_vary/" + vary_name + "=" + str(
                    vary_value) + "/trained_ML_model/trained_classi_model_ML_collect_data_PI.pkl", "rb")
        ...
        ml_bundle = [first_opt]
        if self.strat_cost[first_opt] + self.strat_cost[
            second_opt] <= self.cost_limit and first_opt != second_opt and second_opt != 3:  # DS4 causes system fail early
        ...
        elif self.decision_scheme == 1 or self.decision_scheme == 2 or self.decision_scheme == 6:  # DHEU-based selection scheme
...
class defender_class:
    ...
    def update_defense_impact(self, all_attack_impact, att_strat_list, strat_number):
        ...
        #     self.impact_record[strategy_id] = sum(
        #         [strat_prob[stra] * math.exp(-1 * xi * all_attack_impact[stra]) for stra in np.arange(strat_number)])
        #     # self.impact_record[strategy_id] = math.exp(-1 * xi * sum(all_attack_impact))  # old function
        # omega = 0.5
        xi = 10  # or=10
        gamma = 1
        beta = random.uniform(0, self.uncertainty )
        # di = 0
        # for att_impact in all_attack_impact:
        #     di += math.exp(-1 * xi * att_impact)
        #     if att_impact != 0:
        ...
        response_times = [0.5, 0.7, 1.0, 0.8, 0.9, 1.2, 1.5, 1.1]


        # 更新 impact_record
        for strategy_id in self.chosen_strategy_list:
            self.impact_record[strategy_id] = sum(
                ((1 - omega) * strat_prob[stra] + omega * response_times[stra]) * math.exp(
                    -xi * all_attack_impact[stra])
                for stra in np.arange(strat_number)
            )
            # self.impact_record[strategy_id] = math.exp(-1 * xi * sum(all_attack_impact))  # old function
